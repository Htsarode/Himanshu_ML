...
class EnhancedAnnotationServer:
    ...
    def setup_routes(self):
        ...
        async def process_images(image: UploadFile = File(...)):
            """Process uploaded image for annotation"""
            self.segmented_mask = []
            self.interactive_mask = []
            ...
            self.predictor.set_image(self.GLOBAL_IMAGE)
            
            return JSONResponse(
                content={"message": "Image received successfully"},
                status_code=200,
            )
        ...
        async def click_images(request: Request):
            ...
            try:
                # For videos, we need to set the first frame image for the predictor
                if self.GLOBAL_IMAGE is None and self.VIDEO_PATH:
                ...
                # Predict mask using magic tool (points)
                masks_, scores, logits = self.predictor.predict(
                ...
                if selected_label is not None:
                    ...
                    if selected_label in point_labels:
                        # Optionally, you could post-process the mask to keep only the largest connected component near the click point
                        # For now, just keep the mask as is if the label matches
                        pass
                    else:
        ...
        async def rect_images(
            ...
            try:
                ...
                if self.inference_state is not None and self.VIDEO_PATH:
                    # Video mode - use video predictor
                    _, out_obj_ids, out_mask_logits = self.vid_predictor.add_new_points_or_box(
                        inference_state=self.inference_state,
                    ...
                    # Image mode - use image predictor
                    if self.GLOBAL_IMAGE is None:
                        raise ValueError("No image loaded")

                    # Predict mask using rectangle
                    masks_, scores, logits = self.predictor.predict(
                        point_coords=None,
                        point_labels=None,
                        box=box,
